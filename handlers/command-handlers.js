...
export const commandHandlers = {
    ...
    activated: async ({ chat, msg, client, name }) => {
        const messages = [MESSAGES.welcome(name), MESSAGES.intro, MESSAGES.menuPrompt]
...
async function handleFreeTrial({ chat, msg, client, name }) {
    try {
        await messageService.sendMultiple(chat, msg, client, [MESSAGES.trial.generating, MESSAGES.trial.wait])
        ...
        messageService.scheduleMessage(chat, msg, client, MESSAGES.trial.expired(name), CONFIG.delays.trialExpiry)
    } catch (error) {
